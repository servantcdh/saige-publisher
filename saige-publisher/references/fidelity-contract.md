...
- **DS 컴포넌트를 재사용하면 그 내부 resolved 스타일(radius·padding·shadow·z-index·animation)을 데모와 diff하라.** 다르면 prop/override로 맞추고, 못 맞추면 Design System Compliance Report + Difference Report에 명시. **"DS 컴포넌트 썼으니 됐다"로 내부 델타를 덮지 마라.** *(F1/F2 — 실측: 메뉴 radius 8→4·z-index 40→100·anim 120→150ms가 재사용 뒤에 숨었다)*
...
- **DS 컴포넌트 내부 델타**: 재사용 컴포넌트의 resolved radius/padding/shadow/z-index/animation vs 데모값 — `일치` / `override로 보정` / `미해결(Difference Report 연동)`. 재사용의 은닉 델타를 여기서 강제 노출.
