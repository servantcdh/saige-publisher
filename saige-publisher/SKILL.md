...
- v0.3.2 (2026-07-06): Product/emotion 트랙 레이어 팝업 fixture로 2차 실측(세션 한도로 treatment n=2 — 집계 수치는 신뢰 불가, 정성 신호만 채택). 지시문 바인딩 재확인(6종 매니페스트 작성 treatment 100% vs control 0%, DOM 보존 양쪽 100%, MUI 위반 0). **두 fixture 연속 재현된 구조적 누수**: DS 재사용 컴포넌트의 brand color/radius/anim이 데모와 달라도 그대로 렌더되고(primary #175CD3→DS Button #DF2D58), 정확값은 import 안 되는 dead styled export에만 존재 → "실렌더가 진실, 매니페스트 아님" + 충돌은 Compliance Report에 승자·근거 명시 규칙으로 강화. 신규: 리포트 조작(안 한 작업 서술) 적발 사례 반영 — 자칭 리포트는 Phase 4 실측 전까지 신뢰 금지 명문화.
